# ──────────────────────────────────────────────────────────────
# App_AR_PRED.py  ·  v7  (25-Abr-2025)
# Igual que la v6, **solo** se añade el bloque de “Conclusiones rápidas”
# ──────────────────────────────────────────────────────────────
...
def load_data(fp:Path)->pd.DataFrame:
    ...
    df["AgingBucket"] = pd.cut(
        df["DaysLate"], bins=[-np.inf,0,30,60,np.inf],
        labels=["≤0d","1-30d","31-60d",">60d"]
    )
    return df
...
with st.expander("Matriz de correlación (numéricas)"):
    num_cols = st.multiselect("Columnas numéricas",
...
with st.expander("📌 Conclusiones rápidas"):
    # % importe en los top 10 clientes
    top10_share = (df.groupby("customerID")
                     .InvoiceAmount.sum()
                     .sort_values(ascending=False)[:10].sum()
                   / df.InvoiceAmount.sum()*100) if tot_inv else 0
    # Disputas vs no-disputas en DaysLate
    if df.Disputed.eq(1).any() and df.Disputed.eq(0).any():
        ratio_dl = (df[df.Disputed==1].DaysLate.mean() /
                    df[df.Disputed==0].DaysLate.mean())
    else:
        ratio_dl = np.nan
    # Tendencia >60d últimos 6 meses
    six_months_ago = df["InvoiceDate"].max() - pd.DateOffset(months=6)
    pct_last = (df[(df.InvoiceDate>=six_months_ago) & (df.DaysLate>60)]
                .shape[0] / max(1, df[df.InvoiceDate>=six_months_ago].shape[0])*100)
    pct_prev = (df[(df.InvoiceDate<six_months_ago) & (df.DaysLate>60)]
                .shape[0] / max(1, df[df.InvoiceDate<six_months_ago].shape[0])*100)
    trend = "⬆︎ aumento" if pct_last > pct_prev else "⬇︎ descenso"
    st.markdown(f"""
- **Concentración de riesgo:** El **{top10_share:,.1f}%** del importe total está en los 10 clientes de mayor facturación.  
- **Impacto de disputas:** Las facturas en disputa presentan un *DaysLate* medio **{ratio_dl:,.1f}×** el de las no disputadas.  
- **Tendencia reciente (>60 d):** {trend} de facturas muy atrasadas en los últimos 6 meses.  
""")
